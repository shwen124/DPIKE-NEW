...
class Stat:
    ...
    def cuda_(self):
        ...
        """
        self.to_("cuda" if torch.cuda.is_available() else "cpu")
...
def _unit_test():
    ...
    if args.mode == "cuda" if torch.cuda.is_available() else "cpu":
    ...
    # saved = unbox_numpy_null(numpy.load(f'{testdir}/saved.npz'))
    assert not cs2.qc.device.type == "cuda" if torch.cuda.is_available() else "cpu"
    cs2.to_(device)
    # alldata = alldata.cpu()
    cs2.add(alldata)
    actual_sum *= 2
    ...
    assert not cs.qc.device.type == "cuda" if torch.cuda.is_available() else "cpu"
    cs.to_(device)
    cs.add(alldata)
    # actual_sum *= 2
