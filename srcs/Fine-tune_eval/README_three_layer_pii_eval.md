...
export SFT_ADAPTER_DIR=checkpoints/depn/llama3-8b_lora/2026-05-13_api4_prefix_qlora
...
```bash
LIMIT_PER_TYPE_TRAIN=0 FORCE_RERUN=1 bash srcs/depn/eval/run_three_layer_pii_eval.sh
```
...
python srcs/depn/eval/eval_sft_true_prefix_pii_metrics.py \
  ...
  --adapter_dir checkpoints/depn/llama3-8b_lora/2026-05-13_api4_prefix_qlora \
...
python srcs/depn/eval/compare_pii_eval_layers.py \
  --base outputs/depn/eval/three_layer/base/base_plain_test.json \
  --sft outputs/depn/eval/three_layer/sft/sft_plain_test.json \
  --edited outputs/depn/eval/three_layer/edited/edited_plain_test.json \
  --split test \
  --output_dir outputs/depn/eval/three_layer/comparison
```
