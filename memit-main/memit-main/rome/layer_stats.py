...
def layer_stats(
    ...
    log.info(
        "[layer_stats] layer_name=%s filename=%s exists=%s force_recompute=%s",
        layer_name, filename, filename.exists(), force_recompute,
    )

    if not filename.exists() and download:
        remote_url = f"{REMOTE_ROOT_URL}/data/stats/{file_extension}"
        try:
            log.info("[layer_stats] Attempting to download %s from %s", file_extension, remote_url)
            (stats_dir / "/".join(file_extension.split("/")[:-1])).mkdir(
                exist_ok=True, parents=True
            )
            torch.hub.download_url_to_file(remote_url, filename)
            log.info("[layer_stats] Successfully downloaded.")
        except Exception as e:
            log.warning("[layer_stats] Unable to download: %s. Computing locally.", e)
    ...
    log.info("[layer_stats] tally loop: batch_count=%s (0 => load from cache)", batch_count)
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count):
            ...
            if n_batches > 0 and n_batches % 100 == 0:
                log.info("[layer_stats] processed %d batch groups (%.1fs)", n_batches, time() - t0)

    if n_batches == 0:
        log.info("[layer_stats] loaded from cache (no batches); done in %.1fs", time() - t0)
    else:
        log.info("[layer_stats] computed over %d batch groups; done in %.1fs", n_batches, time() - t0)
